...
class MultiFileGUI:
    def __init__(self, master):
        ...
        # else:
        #     print(f'This is a {os.name} system')

        master.grid_columnconfigure(5, weight=1)

        print(f"height={master.winfo_screenheight()}")
        screen_height = master.winfo_screenheight()
        self.text_area = scrolledtext.ScrolledText(master, wrap=tk.WORD, font=("Courier New", 10),
                                                   height=screen_height//17.77, bg="#ffffff")
        self.text_area.grid(row=0, column=5, rowspan=50, sticky="nsew", padx=10, pady=10)
    ...
    def show_about():
        # version
        messagebox.showinfo("About", "MergeSanger Version: 3.0\nPlease send email to 441394376@qq.com\n"
    ...
    def alignment_two_seq(self, seq1, seq2, save_file):
        ...
        if aligned_pos == -1:
            print("can not find consecutive %d equal in two sequences.", n)
            return -1, -1
    ...
    def align_to_get_boundaries(self, f1, f2, seq1, seq2):
        ...
        alignment_a_left = 0
        alignment_b_left = 0
        output_file_name = f1.split('.')[0] + '_' + f2.split('.')[0] + ".align"
    ...
    def merge_sanger(self):
        ...
        for tup in self.frames:
            ...
            if tup[2].get() == "F":
                ...
                f = open(os.path.basename(tup[1]).split('.')[0] + '.newseq', 'w')
        ...
        for i in range(0, len(self.dna_sequence)-1, 1):
            ...
            newfile1 = os.path.basename(file1).split('.')[0] + '.newseq'
            newfile2 = os.path.basename(file2).split('.')[0] + '.newseq'
        ...
        fullname = os.path.join(self.logdir, merged_file_name)
